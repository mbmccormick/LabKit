...
2. **Build once, deploy that build.** The `build` job (no secrets) runs typecheck, tests and the official validator, then `pnpm build:release --env <env>` writes `out/<env>/`: the web build, the Worker bundle (`worker/index.js`), and `build-manifest.json` (commit, env, workflow run, SHA-256 of the Worker bundle, of every web file and of `_headers`). The bundle and manifest are signed with `actions/attest-build-provenance` (Sigstore). The `deploy` job verifies the attestation, then deploys the prebuilt bundle unchanged (`wrangler deploy --no-bundle`, version tagged with the commit).
...
5. **Deploy-time Worker check.** With a Cloudflare token (the deploy job, or a manual run of `.github/workflows/verify.yml`), the same command also reads the deployed version back from the Cloudflare API and fails unless: one version serves 100% of traffic, it holds exactly the signed Worker bundle and `_headers` file and nothing else, Logpush and observability are off, and there are no tail consumers. The result is in the public Actions log. Changes made later (dashboard edits, secret changes) are not continuously monitored; access controls cover that gap.
